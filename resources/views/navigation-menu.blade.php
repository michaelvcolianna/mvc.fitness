<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:flex">
                    @foreach ($routes as $route)
                        @if ($route->inNav())
                            <x-nav-link :href="route($route->view())"
                                :active="request()->routeIs($route->view())"
                                >{{ $route->name() }}</x-nav-link>
                        @endif
                    @endforeach
                </div>
            </div>
                        ...
                        <x-dropdown align="right" width="60">
                            <x-slot:trigger>
                                <span class="inline-flex rounded-md">
                                    <button type="button"
                                        ...
                                        py-2 border border-transparent text-sm
                            ...
                            <x-slot:content>
                                <div class="w-60">
                                    ...
                                    text-gray-400">Manage Team</div>

                                    <!-- Team Settings -->
                                    <x-dropdown-link :href="route('teams.show', Auth::user()->currentTeam->id)"
                                        >Team Settings</x-dropdown-link>

                                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                                        <x-dropdown-link :href="route('teams.create')"
                                            >Create New Team</x-dropdown-link>
                                    @endcan
                                    ...
                                    @if (Auth::user()->allTeams()->count() > 1)
                                        ...
                                        <div class="block px-4 py-2 text-xs
                                        text-gray-400">Switch Teams</div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @foreach ($routes as $route)
                @if ($route->inNav())
                    <x-responsive-nav-link :href="route($route->view())"
                        :active="request()->routeIs($route->view())"
                        >{{ $route->name() }}</x-responsive-nav-link>
                @endif
            @endforeach
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="flex items-center px-4">
                ...
                <div>
                    <div class="font-medium text-base text-sky-500"
                        >{{ Auth::user()->name }}</div>
                </div>
            </div>
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('stats')"
                    ...
                    >Stats</x-responsive-nav-link>

                <!-- Account Management -->
                <x-responsive-nav-link :href="route('profile.show')"
                    :active="request()->routeIs('profile.show')"
                    >Profile</x-responsive-nav-link>

                @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                    <x-responsive-nav-link :href="route('api-tokens.index')"
                        :active="request()->routeIs('api-tokens.index')"
                        >API Tokens</x-responsive-nav-link>
                @endif
                ...
                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                    ...
                    <x-responsive-nav-link :href="route('teams.show', Auth::user()->currentTeam->id)"
                        ...
                        >Team Settings</x-responsive-nav-link>

                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                        <x-responsive-nav-link :href="route('teams.create')"
                            :active="request()->routeIs('teams.create')"
                            >Create New Team</x-responsive-nav-link>
                    @endcan
